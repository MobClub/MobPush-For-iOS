...
{
    [MobPush addLocalNotification:nil result:^(id result, NSError *error) {
...
{
    [MobPush addLocalNotification:@"" result:^(id result, NSError *error) {
...
- (void)testSetBadge
{
    ...
    [self _wait:10 testOperation:^(void (^terminate)(void)) {
        [MobPush getBadgeWithhandler:^(NSInteger badge, NSError *error) {
            XCTAssert(badge!=10,@"badge Count unmatched");
            terminate();
        }];
    }];
}
