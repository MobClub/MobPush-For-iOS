 ...
 * @param handler 结果回调
 */
+ (void)addCustomParamsWith:(NSDictionary *)params
                    handler:(void(^)(NSArray *successKeys, NSArray *failedKeys, NSError *error))handler;
 ...
 */
+ (void)deleteCustomParamsWith:(NSDictionary *)params
                       handler:(void(^)(NSArray *successKeys, NSArray *failedKeys, NSError *error))handler;
...
/// 设置用户使用语言
/// - Parameter lan: 语言编码(zh-Hans、zh-Hant等)
+ (void)setUserLanguage:(NSString *)lan;
